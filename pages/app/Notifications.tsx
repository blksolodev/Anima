...
import { Heart, MessageCircle, UserPlus, AtSign, Loader2, BellOff } from 'lucide-react';
...
export const Notifications: React.FC = () => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      default: return <AtSign size={20} className="text-[#F4D03F]" />;
    }
  };
    ...
    <div className="w-full max-w-2xl mx-auto pt-6 px-4 md:px-6">
      ...
      <div className="space-y-3">
        {loading ? (
            ...
            <GlassCard 
                key={notif.id} 
                className={`!p-4 flex gap-4 transition-colors ${notif.read ? 'opacity-70' : 'bg-white/10'}`}
            >
                <div className="mt-1">
                {getIcon(notif.type)}
                </div>
                ...
                <div className="flex items-center gap-2 mb-1">
                    <div className="w-8 h-8 rounded-full bg-white/10 overflow-hidden">
                        {notif.fromUser?.avatar && <img src={notif.fromUser.avatar} alt="" className="w-full h-full object-cover" />}
                    </div>
                    <span className="font-bold">{notif.fromUserId}</span> 
                ...
                <p className="text-white/90">
                    {notif.content || (notif.type === 'follow' ? 'started following you' : 'interacted with you')}
                </p>
                </div>
                {!notif.read && (
